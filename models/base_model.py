...
class BasicClassifier(BasicModel):
    ...
    def __init__(
        ...
        self.auc_roc = nn.ModuleDict(
            {state: AUROC(**aucroc_kwargs) for state in ["train_", "val_", "test_"]}
        )
        self.acc = nn.ModuleDict(
            {state: Accuracy(**acc_kwargs) for state in ["train_", "val_", "test_"]}
        )
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int):
        ...
        target_squeezed = torch.squeeze(
            target, 1
        )  # TODO Why is this necessary and is it the right thing to do?
